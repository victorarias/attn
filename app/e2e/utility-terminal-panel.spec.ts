...
// Helper to collapse terminal panel
async function collapseTerminalPanel(page: import('@playwright/test').Page, sessionId: string) {
  await page.evaluate((id) => {
    window.__TEST_COLLAPSE_TERMINAL_PANEL?.(id);
  }, sessionId);
}

// Helper to set active terminal
async function setActiveTerminal(page: import('@playwright/test').Page, sessionId: string, terminalId: string) {
  await page.evaluate(
    ({ sessionId, terminalId }) => {
      window.__TEST_SET_ACTIVE_TERMINAL?.(sessionId, terminalId);
    },
    { sessionId, terminalId }
  );
}

// Helper to remove terminal
async function removeTerminal(page: import('@playwright/test').Page, sessionId: string, terminalId: string) {
  await page.evaluate(
    ({ sessionId, terminalId }) => {
      window.__TEST_REMOVE_TERMINAL?.(sessionId, terminalId);
    },
    { sessionId, terminalId }
  );
}
...
test.describe('Utility Terminal Panel', () => {
  ...
  test.describe('Keyboard Shortcuts', () => {
    ...
    test('⌘⇧{ switches to previous tab', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      // Second tab should be active (last added)
      const tabs = page.locator('.terminal-tab');
      await expect(tabs.nth(1)).toHaveClass(/active/);

      // ⌘⇧{ should switch to previous tab - dispatch with exact key value
      await dispatchShortcut(page, '{', { meta: true, shift: true });
      await expect(tabs.nth(0)).toHaveClass(/active/);
    });

    test('⌘⇧} switches to next tab', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      // Switch to first tab
      await setActiveTerminal(page, 's1', 'term-1');
      const tabs = page.locator('.terminal-tab');
      await expect(tabs.nth(0)).toHaveClass(/active/);

      // ⌘⇧} should switch to next tab - dispatch with exact key value
      await dispatchShortcut(page, '}', { meta: true, shift: true });
      await expect(tabs.nth(1)).toHaveClass(/active/);
    });

    test('⌘T creates new terminal when panel is open', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      // Add mock terminal and open panel
      await addMockTerminal(page, 's1', 'term-1', 'Shell 1');
      await expect(page.locator('.utility-terminal-panel')).toBeVisible();
      await expect(page.locator('.terminal-tab')).toHaveCount(1);

      // Note: ⌘T actual terminal creation requires Tauri invoke which doesn't work in E2E
      // The new tab button click test below verifies the UI works
    });
  ...
  test.describe('UI Interactions', () => {
    test('clicking tab selects it', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      const tabs = page.locator('.terminal-tab');
      // Second tab should be active initially
      await expect(tabs.nth(1)).toHaveClass(/active/);
    ...
    test('close button on tab closes it', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      const closeButton = page.locator('.terminal-tab').nth(0).locator('.terminal-tab-close');
      await closeButton.click();

      await expect(page.locator('.terminal-tab')).toHaveCount(1, { timeout: 2000 });
    });

    test('collapse button collapses the panel', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      await page.locator('.terminal-collapse').click();
      await expect(page.locator('.utility-terminal-panel')).not.toBeVisible({ timeout: 2000 });
    });

    test('double-click tab opens rename input', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      // Double-click the tab title
      const tabTitle = page.locator('.terminal-tab .terminal-tab-title');
      await tabTitle.dblclick();

      // Should show an input for renaming
      await expect(page.locator('.terminal-tab input')).toBeVisible({ timeout: 1000 });
    });

    test('rename input updates tab title on Enter', async ({ page, daemon }) => {
      await daemon.start();
      await page.goto('/');
      await page.waitForSelector('.dashboard');
      ...
      const input = page.locator('.terminal-tab input');
      await input.fill('My Custom Shell');
      await input.press('Enter');

      // Title should be updated
      await expect(page.locator('.terminal-tab .terminal-tab-title')).toHaveText('My Custom Shell');
    });
  });
