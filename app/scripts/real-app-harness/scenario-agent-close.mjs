...
// The whole scenario runs in a few seconds, which is unwatchable. Only a
// recording run pays for the pauses.
...
// A pane wraps at its own width, so every read compares with the whitespace
...
// The marker appears twice — as typed and again as the shell prints it — so the
// output is what lies between them.
...
async function main() {
  ...
  try {
    ...
    delegate = await runner.step('dispatch_a_delegate', async () => {
      ...
      // The spawn takes the screen, so come back and let the dispatcher's shell
      // reach its prompt: a close typed into a pane still running never runs.
