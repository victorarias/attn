...
export async function waitForRemoteProcessesByHarnessRoot(target, harnessRoot, predicate, description, timeoutMs = 60_000) {
  ...
  let lastSnapshot = [];
  while (Date.now() - startedAt < timeoutMs) {
    lastSnapshot = await listRemoteProcessesByHarnessRoot(target, harnessRoot, Math.min(30_000, timeoutMs));
    const value = predicate(lastSnapshot);
    if (value) {
      return value;
    }
  ...
  throw new Error(
    `Timed out waiting for ${description}. Last remote process snapshot for harness root ${harnessRoot}:\n${JSON.stringify(lastSnapshot, null, 2)}`
  );
}
