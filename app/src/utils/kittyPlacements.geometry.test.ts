...
describe('placementQuad', () => {
  it('places a fully visible image at its cell origin, at its rendered size', () => {
    const quad = placementQuad(placed({ bufferRow: 103, col: 4 }), 100, CELL_W, CELL_H, GRID_W, GRID_H);
  ...
  it('clips an image that starts above the viewport, and crops rather than squashes', () => {
    ...
    const quad = placementQuad(placed({ bufferRow: 98 }), 100, CELL_W, CELL_H, GRID_W, GRID_H);
  ...
  it('clips an image that runs past the right edge of the grid', () => {
    ...
    const quad = placementQuad(placed({ bufferRow: 100, col: 74 }), 100, CELL_W, CELL_H, GRID_W, GRID_H);
  ...
  it('clips an image that runs past the bottom of the grid', () => {
    const quad = placementQuad(placed({ bufferRow: 123 }), 100, CELL_W, CELL_H, GRID_W, GRID_H);
  ...
  it('reports no quad for an image scrolled entirely out of view', () => {
    expect(placementQuad(placed({ bufferRow: 95 }), 100, CELL_W, CELL_H, GRID_W, GRID_H)).toBeNull();
    expect(placementQuad(placed({ bufferRow: 124 }), 100, CELL_W, CELL_H, GRID_W, GRID_H)).toBeNull();
  });

  it('keeps an image that only just pokes into the viewport', () => {
    // Its last 20 pixels sit on the first visible row.
    const quad = placementQuad(placed({ bufferRow: 97 }), 100, CELL_W, CELL_H, GRID_W, GRID_H);
...
describe('placementSourceRect', () => {
  it('samples the whole texture when the placement names no source rect', () => {
    const quad = placementQuad(placed(), 100, CELL_W, CELL_H, GRID_W, GRID_H)!;
  ...
  it('samples the sub-rect a placement names', () => {
    const quad = placementQuad(
      ...
      100, CELL_W, CELL_H, GRID_W, GRID_H,
    )!;
