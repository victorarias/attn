...
use attn_protocol::{
    BootstrapWorkspaceInitialSession, BootstrapWorkspaceMessage, KillSessionMessage, MuteMessage,
    PtyInputMessage, RegisterWorkspaceMessage, SpawnSessionMessage, UnregisterWorkspaceMessage,
    WorkspaceLayoutClosePaneMessage, WorkspaceLayoutFocusPaneMessage, WorkspaceLayoutPaneKind,
    WorkspaceLayoutSplitDirection, WorkspaceLayoutSplitPaneMessage,
};
...
fn handle_action(
    ...
    match action {
        ...
        "get_launcher_state" => read_app(app, cx, |app, cx| app.automation_launcher_snapshot(cx)),
        ...
        "bootstrap_workspace" => bootstrap_workspace(app, cx, payload),
        ...
        "open_new_workspace_launcher" => open_new_workspace_launcher(app, cx),
        "open_add_pane_launcher" => open_add_pane_launcher(app, cx, payload),
        "cancel_launcher" => cancel_launcher(app, cx),
        "set_launcher_path" => set_launcher_path(app, cx, payload),
        "inspect_launcher_path" => inspect_launcher_path(app, cx),
...
fn bootstrap_workspace(
    ...
    let agent = optional_string(&payload, &["agent"]).unwrap_or_else(|| "shell".into());
    let shell = agent == "shell";
    let message = BootstrapWorkspaceMessage {
        cmd: "bootstrap_workspace",
        id: workspace_id.clone(),
        title: optional_string(&payload, &["title"])
            .unwrap_or_else(|| "Automation Workspace".into()),
        directory: directory.clone(),
        endpoint_id: None,
        initial_session: BootstrapWorkspaceInitialSession {
            id: session_id.clone(),
            cwd: optional_string(&payload, &["cwd"]).unwrap_or_else(|| directory.clone()),
            kind: if shell {
                WorkspaceLayoutPaneKind::Shell
            } else {
                WorkspaceLayoutPaneKind::Agent
            },
            agent: Some(agent.clone()),
            cols: payload.get("cols").and_then(Value::as_u64).unwrap_or(100) as u16,
            rows: payload.get("rows").and_then(Value::as_u64).unwrap_or(36) as u16,
            label: optional_string(&payload, &["label"]),
            yolo_mode: payload.get("yolo_mode").and_then(Value::as_bool),
            executable: optional_string(&payload, &["executable"]),
        },
    };
    send_daemon(app, cx, &message)?;
    events::record(
        "workspace_bootstrap_requested",
        json!({"workspace_id": workspace_id, "session_id": session_id, "agent": agent}),
    );
    Ok(json!({"workspace_id": workspace_id, "session_id": session_id}))
}
...
fn spawn_session(
    ...
    message.yolo_mode = payload.get("yolo_mode").and_then(Value::as_bool);
    message.target_pane_id = optional_string(
        &payload,
        &["targetPaneId", "target_pane_id", "paneId", "pane_id"],
    );
    message.direction = match optional_string(&payload, &["direction"]).as_deref() {
        Some("horizontal") => Some(WorkspaceLayoutSplitDirection::Horizontal),
        Some("vertical") => Some(WorkspaceLayoutSplitDirection::Vertical),
...
fn open_new_workspace_launcher(
    app: &WeakEntity<NativeApp>,
    cx: &mut AsyncApp,
) -> Result<Value, String> {
    let entity = app.upgrade().ok_or("native app dropped")?;
    cx.update_entity(&entity, |native, cx| {
        native.automation_open_new_workspace(cx);
    })
    .map_err(|error| format!("open new workspace launcher: {error}"))?;
    Ok(json!({"open": true, "mode": "new_workspace"}))
}

fn open_add_pane_launcher(
    app: &WeakEntity<NativeApp>,
    cx: &mut AsyncApp,
    payload: Value,
) -> Result<Value, String> {
    ...
    let entity = app.upgrade().ok_or("native app dropped")?;
    cx.update_entity(&entity, |native, cx| {
        native.automation_open_add_pane(direction, cx);
    })
    .map_err(|error| format!("open add pane launcher: {error}"))?;
    Ok(json!({"open": true, "mode": "add_pane"}))
}

fn cancel_launcher(app: &WeakEntity<NativeApp>, cx: &mut AsyncApp) -> Result<Value, String> {
    let entity = app.upgrade().ok_or("native app dropped")?;
    cx.update_entity(&entity, |native, cx| {
        native.automation_cancel_launcher(cx);
    })
    .map_err(|error| format!("cancel launcher: {error}"))?;
    Ok(json!({"open": false}))
}

fn set_launcher_path(
    app: &WeakEntity<NativeApp>,
    cx: &mut AsyncApp,
    payload: Value,
) -> Result<Value, String> {
    ...
    let entity = app.upgrade().ok_or("native app dropped")?;
    cx.update_entity(&entity, |native, cx| {
        native.automation_set_launcher_path(path.clone(), cx)
    })
    .map_err(|error| format!("set launcher path: {error}"))??;
    Ok(json!({"path": path}))
}

fn inspect_launcher_path(app: &WeakEntity<NativeApp>, cx: &mut AsyncApp) -> Result<Value, String> {
    let entity = app.upgrade().ok_or("native app dropped")?;
    cx.update_entity(&entity, |native, cx| {
        native.automation_inspect_launcher_path(cx)
    })
    .map_err(|error| format!("inspect launcher path: {error}"))??;
    Ok(json!({"inspectionRequested": true}))
}
