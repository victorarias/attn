...
100 real ticket descriptions/comments -> opus rewrite under the /bro nudge ->
...
NUDGE = "Restate your last message. Stop using jargon and speak coherently. State it more simply and concisely, like one human talking to another. Keep what carries information the prose cannot — diagrams, tables, code, commands, links. Improve them if you can; just don't drop them. Keep every number, measurement, and file or line reference, and keep stated uncertainty stated — a receipt or a caveat is information, not clutter."
...
rows = []
with open("../ticketreg/tickets.jsonl") as f:
    for line in f:
        r = json.loads(line)
        r["file"], r["line"] = r["kind"], 0
        rows.append(r)
sample = rows
print(f"pipeline over {len(sample)} real ticket descriptions/comments", flush=True)

out = open("exp4.jsonl", "w")
def run(r):
