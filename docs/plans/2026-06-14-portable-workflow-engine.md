...
**Worktree isolation is the SAFE resume mode (E6).** On resume, a journaled cache-hit (`ordinal` AND `prompt_hash` AND `schema_hash` all match — the predicate is **unchanged** by isolation) replays the journaled *result* and does **NOT** re-create the worktree or re-run the subagent. The throwaway worktree is already gone (it was auto-removed if clean, or kept as the side effect of the *original* live run); a replay observes neither — it only sees the journaled structured result. Isolation changes **where a live call runs**, not **what its cached result is**, so it is *never* part of the cache identity. This is why worktree-isolated is the safe resume mode: there is no shared mutable state to drift, and a cache-hit performs zero filesystem work.
...
- **Worktree isolation** (`isolation:'worktree'`). Opt-in for parallel mutation; hand the subagent a fresh worktree as CWD over attn's existing worktree infra. The structured result still returns via `return_result`; mutations are the consumed side effect. Pairs with the §6 side-effect contract.

  **Implemented cleanup rule (E6).** Per isolated `agent()` call: resolve the repo root from the run's working tree (`git.ResolveMainRepoPath`), create a fresh worktree on a unique branch derived from the call's structural ordinal (`attn-wf/<short-ordinal-hash>` + `GenerateWorktreePath`), and run the subagent with that worktree as CWD (sandbox stays `workspace-write`; scratch schema/result files stay under `runTmpDir`, never the worktree, so cleanliness reflects only the agent's own edits). **After the run — success OR failure — the lifecycle is outcome-independent and decided by `git status --porcelain --untracked-files=all`:**
  - **unchanged (git-clean) → auto-removed.** `DeleteWorktree(..., force)` + best-effort branch prune. A no-op isolated call leaves nothing behind.
  - **changed (dirty, incl. untracked) → KEPT.** The mutations are the consumed side effect and must not be discarded; the retained worktree path is logged. This holds even when the run itself *failed* (a partial mutation is preserved for inspection, never silently dropped).

  On `CreateWorktree` failure the call **fails closed** (returns a Go error → engine resolves the agent() promise to `null`); it never falls back to the shared working tree, which would defeat isolation. `isolation:''` (none) is byte-identical to the E3 writable default (CWD = the shared working tree, no worktree created). `model` is honored as a per-call model override; `agentType` is carried for parity but currently unused.
