...
- Diff detail keeps selected-file content visible and caps background viewed-file checks.
...
| Active session git status subscription | Selecting a session | `gitCoordinator.Status`: `git status --porcelain -z --untracked-files=all`, limited tracked-only fallback, `check-ignore` | No direct indicator; drives downstream changes state | No | Full untracked scans can be expensive, but refreshes are coalesced and slowed after costly runs | Background with adaptive cadence. Do not show every poll. Expose "status stale" only after repeated failures or when user opens changes. |
...
| Diff detail panel initial file list | Opening diff detail panel | `sendFetchRemotes`, then daemon-owned `get_branch_diff_files` snapshot | Panel-level loading/empty/error behavior | Yes, for reviewing | User intentionally opened review; they need to know why files are unavailable | Local panel progress. Use daemon snapshots; escalate timeout/error in panel, not globally. |
...
| R3 | Active session git status subscription | Frontend enters session view and active session has a directory; daemon sends immediate result, coalesces dirty refreshes, and slows safety refreshes after expensive runs | `app/src/App.tsx` active-session subscribe effect; `internal/daemon/ws_git.go` scheduler; `internal/daemon/git_coordinator.go` `Status`; `internal/daemon/git_coordinator_status.go` status parser | `git status --porcelain -z --untracked-files=all`; tracked-only fallback after full status timeout; optional filesystem walk for collapsed untracked dirs; `git check-ignore --stdin` | Daemon hashes and suppresses unchanged events, shares concurrent status work per repo/mode, and marks limited tracked-only results. | Silent active-session background. Do not show every poll. Expose "status stale" only after repeated failures or when user opens changes. |
| R4 | Changes panel branch diff, initial and interval refresh | Session view with active repo directory while Changes panel is visible; immediate visible refresh and interval refresh | `app/src/App.tsx` `refreshBranchDiff` interval effect | Daemon-owned `get_branch_diff_files`; coordinator returns cached branch snapshot while refreshing and invalidates cache on refresh failure | Hidden panel does not schedule branch diff refreshes. Visible panel keeps previous result after first load and exposes loading/refreshing/error only when `ChangesPanel` renders. | Panel-owned data. Continue using daemon snapshots; tune first-load UX only if slow repos make the empty state confusing. |
| R5 | Changes panel branch diff on status update | `git_status_update` for the active repo while Changes panel is visible | `app/src/App.tsx` gitStatus effect schedules debounced `refreshBranchDiff` | Same as R4 | Status bursts are debounced/coalesced, and slow branch diff refreshes set a cooldown before the next queued refresh. | Panel-owned reactive refresh. Keep stale data visible; do not add global UX. |
| R6 | Diff detail branch file list, local refs path | Opening Diff Detail panel; delayed 150ms unless remote sync wins | `app/src/components/DiffDetailPanel.tsx` open effect `runLocalDiff` | Daemon-owned `get_branch_diff_files` with default base ref | Panel shows loading while waiting; local result is ignored if newer remote-ref result lands first. | User-visible panel load. Do not skip when panel is open; rely on daemon snapshots rather than client branch-list caches. |
| R7 | Diff detail remote sync and refreshed branch file list | Opening Diff Detail panel | `DiffDetailPanel` open effect `sendFetchRemotes(...).then(sendGetBranchDiffFiles)` | `git fetch --all --prune`, then same daemon branch diff as R6 | Shows panel sync state/warning. Local branch diff can render before remote refresh completes. | User-visible panel sync. Keep local fallback; consider making fetch opt-in/backed off for slow repos later. |
...
| R9 | Diff detail viewed-file change detection | Active-session `git_status_update` while Diff Detail panel has viewed files | `DiffDetailPanel` viewed-files effect | Background `get_file_diff` checks for viewed non-selected files still in the file list | Silent background checks are debounced, capped, coalesced per path, and rerun once if another status arrives while a path is in flight. Errors are ignored; only changed badge is surfaced. | Silent panel background. Remaining tuning is cadence/backoff, not fan-out correctness. |
...
- R3 active-session git status polling, after measuring real slow-repo cadence.
- R9 viewed-file change detection cadence/backoff.
...
- R4/R5 hidden Changes refreshes are already avoided; keep future work panel-owned.
- Measure R3 before changing cadence further. It feeds several downstream behaviors, so backing it off blindly can make the app feel stale in surprising places.
- R9 now has in-flight/concurrency guards; consider cadence/backoff only after measuring slow repos.
