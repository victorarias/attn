...
### R5 — Hook seam (with agent edits, PR4) — DONE

Every `PostToolUse` matcher generated by `internal/hooks` called `_hook-state` and discarded
the JSON payload. The naive move for agent-edit tracking — a second hook entry — would add a
second process spawn on *every* tool call. Built as designed instead: one `_hook-tool-use`
that sets the working state and records the touched path from the same payload, replacing
the catch-all `_hook-state` entry for both agents.
...
- PR4 (this PR): agent-edited markdown as a ranking signal. The catch-all
  `PostToolUse` hook becomes `_hook-tool-use` for both agents (R5), which sets
  the working state and records markdown the call wrote in the same spawn:
  Claude names the file in `tool_input.file_path`, Codex routes every write
  through `apply_patch` whose patch headers carry the paths. Recorded as
  `source = "edited"`, so it introduces files nobody has opened. `recent_files`
  now merges the sources into one entry per file, weights an edit below an open,
  and takes a `root` so the workspace in front of you ranks first. Protocol 187.
...
already has two consumers, and carries R4. R3 lands with path mode in PR3. R5 lands
with agent-edit tracking in PR4.

### Agent-edit coverage (PR4)

Verified against Claude Code 2.x and codex-cli 0.145.0 by capturing real `PostToolUse`
payloads. Both send `tool_name`, `tool_input`, and `cwd`; they describe an edit
differently, and the coverage that follows is deliberate:

- Claude: `Edit` / `Write` / `MultiEdit` / `NotebookEdit`, path in `tool_input.file_path`
  (`notebook_path` for notebooks).
- Codex: `apply_patch`, paths in the `*** Add File:` / `*** Update File:` /
  `*** Move to:` headers of `tool_input.command`. A rename is recorded at its
  destination; a deletion is not recorded at all.
- Not covered: a file rewritten through the shell (`sed -i`, a heredoc) arrives as an
  ordinary `Bash` call with no attributable path. Accepted — parsing shell commands for
  file writes is guesswork.
- Codex only runs a hook whose recorded `trusted_hash` matches, so the hash entry moves
  with the command; a live codex session is the only thing that proves it.
