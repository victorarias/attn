...
- Open questions 3 and 4 (classifier prompt, resume argument mapping)
...
│   ├── driver.ts             # driver.register + driver.spawn/resume handlers
│   ├── state.ts              # map pi extension events → attn state reports
│   ├── classifier.ts         # shells out to `pi -p` for WAITING vs DONE
│   ├── resume.ts             # resume flag mapping + session-file path resolution
...
Plugin:
- Connects to `~/.attn/attn.sock`, sends the base `hello` handshake. The driver-specific registration method is defined when that phase lands rather than encoded as a generic role field.
- Registers for agent `pi` with `capabilities: { resume: true, yolo: false, classifier: true, state_reporting: true }`.
- Handles `driver.spawn` and `driver.resume`: returns `{ argv, env, cwd }`. Includes `-e <staged-extension-path>` in argv even though the extension is a no-op in this phase.

Resume argument mapping follows the old plan §C. Phase 1 honest scope:
- New session: works
- `--resume` picker: works
...
**Deliverable:** `attn -s foo` with agent=pi works and resume picker works. Plugin installable via `attn plugin install --path ./attn-pi` (or dev mode).
...
1. Store lookup via a future explicit `session.get_metadata { session_id }` plugin surface.
