...
- [ ] **PR1 — Notebook core: format + daemon service (~900).** [was old PR1 + PR2]
      ...
      default), atomic temp+rename writer, hash-CAS edit, and a `notebook_changed` event on
      every attn-originated write. Protocol `notebook_list`/`notebook_read`/`notebook_write`/
      ...
      broadcast) tests. **Delivers a real, browsable, daemon-owned journal+memory store.**
      *Scoping refinement:* external-edit **fsnotify reconcile** moved to PR3, alongside its
      only consumer (the live UI); the daemon already emits `notebook_changed` on its own
      writes, so nothing observable is lost before then.
...
- [ ] **PR3 — Markdown UI: read + edit + send-to-chief (~1400).** [was old PR4 + PR5]
      ...
      (incl. external-edit **fsnotify reconcile**: the daemon watches `notebook.root`, reloads
      on external/Obsidian writes, and re-emits `notebook_changed`), daemon-mediated;
      **tile** (workspace pane) + **fullscreen** modes (preserve open file
