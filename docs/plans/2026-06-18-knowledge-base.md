...
Status: in progress (stacked on `feat/kill-dreaming`)
Date: 2026-06-18
...
## Stack (each PR on the prior; base `feat/kill-dreaming`)
...
- Doorbell: rewrite `notebookActivationPrompt` and `chiefInboxNudgePrompt` to
  native-file pointers; update `notebook_test.go` doorbell assertions, `main_test.go`,
  `hooks_test.go` (drop CLI-verb assertions that PR3 replaces).

### PR3 — `feat/chief-roleplay-guidance`: roleplay guidance + keeper archiving + docs
- `internal/hooks/hooks.go` — rewrite `NotebookGuidance` as the chief roleplay
  (knowledge base, PARA, OKF `type`, edit-directly, promote→areas, drop
  "write through daemon"). Rewrite `hooks_test.go` accordingly.
...
- **Live-promotion guidance parity.** PR2's doorbell points a freshly-promoted
  chief at `<root>/index.md` (structure) while the full roleplay guidance arrives
  in the system prompt only at launch. If you want the *live-promotion* path to
  also get the full guidance (today's behavior via `attn notebook guide`), the
  daemon can write `hooks.NotebookGuidance` into a reserved file at promotion and
  point there. Default: index.md is enough (non-determinism accepted).
