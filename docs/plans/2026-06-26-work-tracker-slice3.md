...
| 3d-2 | `tickets/slice-3d-notify` | ✅ additive | ✅ **built** — real `Nudger` (`typeDoorbell` fixed trigger) + `notifyTicketObservers(ticketID)` run after the status / crash producers (and flushed when a session goes idle), deciding Watch (Claude) / Nudge (idle codex) / Deferred via `TicketParticipants`. (Created-event notify is skipped — the brief reaches the agent via its spawn prompt; chief-comment producer lands with slice 4.) |
| 3d-3 | `tickets/slice-3d-e2e` | ⚠️ **entangled with cutover** | **Finding:** switching the agent's standing prompt to `attn ticket …` *only* would strand the existing dispatch-based chief view (it reads dispatches, not tickets) until the chief reads tickets — i.e. slice 4 + the 3e removal. So the prompt rewire belongs with the cutover, not as a standalone additive step. The real-Claude e2e is best run once slice 4 gives a **visible board** to observe. → 3d-3's additive remainder is empty; its e2e folds into slice-4 verification, its prompt rewire into 3e. |
