...
- Closed session: a session the user or an agent ended. The row and everything it owns stay in the ledger, marked with when it closed, who closed it and, for an agent close, why. It leaves the sidebar, the queue and every other live surface at once; `attn session list --closed` and `attn session show` are how it is read back. Distinct from Reaped, which deletes. Garden Resume runs a closed tender under its own id again and lifts the close to do it; a resume that then fails puts the close back exactly, closed_at included.
