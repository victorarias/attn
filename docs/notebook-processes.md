...
- **Hand-authored only.** There is no automated writer or promote pass today —
  the dreaming harvest that would have fed one was removed. Notes are written two
  ways: the frontend's in-app editor over the WebSocket path
  (`sendNotebookWriteWSResult`/`Store.Write`, `internal/daemon/notebook.go`), and
...
- **The chief of staff consumes it.** The launch guidance
  (`internal/hooks/hooks.go`) directs the chief to orient by reading the
  notebook's `index.md` and `knowledge/index.md` before substantive work; a
  live-promotion doorbell points it at `<root>/index.md`.
