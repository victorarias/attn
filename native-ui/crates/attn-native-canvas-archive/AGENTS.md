# native-ui canvas archive

Archived architecture conventions and learnings for the discarded GPUI native
canvas client. Reuse mechanics selectively; do not treat its canvas model or
protocol subset as the active native-client direction.

## Architecture

The native app follows **MVVM with a hexagonal edge**. Four directories under
`crates/attn-native-canvas-archive/src/`, each with one responsibility:
...
`crates/attn-native-canvas-archive/src/theme.rs`. The visual language is
documented in plates at
[`docs/prototypes/archive/native-canvas/native-design-system-observatory.html`](../../../../docs/prototypes/archive/native-canvas/native-design-system-observatory.html)
