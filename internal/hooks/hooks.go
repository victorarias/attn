...
- Delegation hands work off — it doesn't block you. When you delegate, attn opens a tracked ticket bound to that session and moves it across a board (Working, Blocked, In Review, Done, Failed, Crashed) as the agent self-reports. Right after delegating, arm a harness Monitor running `+"`"+`attn ticket inbox --watch`+"`"+` so a ticket's state-changes push to you the moment they land instead of you polling the board. Record the delegation in the journal, report back to the user, and your turn is done until a watched ticket pushes or the user re-engages you.
