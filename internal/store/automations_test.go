...
// TestListPrunableAutomationRunsGatesOnUpdatedAtNotCreatedAt pins that the
// age floor is measured from updated_at (when the run became terminal), not
// created_at (when it was claimed). A run claimed long ago but delivered
// just now must not be immediately prunable — its shared worktree would
// otherwise be destroyable minutes after the work finished.
func TestListPrunableAutomationRunsGatesOnUpdatedAtNotCreatedAt(t *testing.T) {
	s := New()
	claimedAt := time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC)
	deliveredAt := time.Date(2026, 7, 20, 3, 0, 0, 0, time.UTC)
	def, err := s.UpsertAutomationDefinition("nightly", "Nightly", `{"id":"nightly"}`, true, claimedAt)
	if err != nil {
		t.Fatal(err)
	}
	ids := AutomationRunReservation{RunID: "run-1", OccurrenceID: "occ-1", TicketID: "ticket-1", SessionID: "session-1", WorkspaceID: "workspace-1", PaneID: "pane-1"}
	run, created, err := s.ClaimManualAutomationRun(def.ID, "request-1", "", `{}`, def.Revision, `{}`, claimedAt, ids)
	if err != nil || !created {
		t.Fatalf("claim created=%v err=%v", created, err)
	}
	if err := s.MarkAutomationRunDelivered(run.ID, "{}", deliveredAt); err != nil {
		t.Fatal(err)
	}

	// olderThan sits well after claimedAt but before deliveredAt: a
	// created_at-gated query would treat this run as prunable, an
	// updated_at-gated one must not.
	prunable, err := s.ListPrunableAutomationRuns(def.ID, 0, deliveredAt.Add(-time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if len(prunable) != 0 {
		t.Fatalf("expected a just-delivered run to be protected by the updated_at age gate, got %#v", prunable)
	}

	// Once olderThan passes deliveredAt, the run is a genuine candidate.
	prunable, err = s.ListPrunableAutomationRuns(def.ID, 0, deliveredAt.Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if len(prunable) != 1 || prunable[0].ID != run.ID {
		t.Fatalf("expected the run to become prunable once the age floor passes its updated_at, got %#v", prunable)
	}
}

// TestListPrunableAutomationRunsKeepWindowTiebreaksByID pins the id DESC
// tiebreaker: two runs minted in the same second (RFC3339 second resolution
// loses sub-second ordering) must still select a deterministic newest-N set
// for the keep window, not an arbitrary one.
func TestListPrunableAutomationRunsKeepWindowTiebreaksByID(t *testing.T) {
	s := New()
	now := time.Date(2026, 7, 20, 3, 0, 0, 0, time.UTC)
	def, err := s.UpsertAutomationDefinition("nightly", "Nightly", `{"id":"nightly"}`, true, now)
	if err != nil {
	...
	idsA := AutomationRunReservation{RunID: "run-a", OccurrenceID: "occ-a", TicketID: "ticket-a", SessionID: "session-a", WorkspaceID: "workspace-a", PaneID: "pane-a"}
	runA, created, err := s.ClaimManualAutomationRun(def.ID, "request-a", "", `{}`, def.Revision, `{}`, now, idsA)
	if err != nil || !created {
		t.Fatalf("claim a created=%v err=%v", created, err)
	}
	if err := s.MarkAutomationRunDelivered(runA.ID, "{}", now); err != nil {
		t.Fatal(err)
	}
	idsB := AutomationRunReservation{RunID: "run-b", OccurrenceID: "occ-b", TicketID: "ticket-b", SessionID: "session-b", WorkspaceID: "workspace-b", PaneID: "pane-b"}
	runB, created, err := s.ClaimManualAutomationRun(def.ID, "request-b", "", `{}`, def.Revision, `{}`, now, idsB)
	if err != nil || !created {
		t.Fatalf("claim b created=%v err=%v", created, err)
	}
	if err := s.MarkAutomationRunDelivered(runB.ID, "{}", now); err != nil {
		t.Fatal(err)
	}

	// Both runs share created_at and updated_at exactly; keep=1 must
	// deterministically protect the id-DESC-greatest ("run-b") and leave the
	// other ("run-a") prunable.
	prunable, err := s.ListPrunableAutomationRuns(def.ID, 1, now.Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if len(prunable) != 1 || prunable[0].ID != runA.ID {
		t.Fatalf("expected the id DESC tiebreak to deterministically protect run-b and prune run-a, got %#v", prunable)
	}
}
