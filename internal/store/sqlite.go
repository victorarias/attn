...
CREATE TABLE IF NOT EXISTS app_reconcile_progress (
	...
	// Applied by applyMigration109, whose ALTERs are column-guarded. Besides
	...
	{109, "record app reconcile invocation lifecycles", ``},
}
...
func migrateDB(db *sql.DB, dbPath string) error {
	...
	for _, m := range migrations {
		...
		} else if m.version == 109 {
			if err := applyMigration109(tx); err != nil {
				tx.Rollback()
				return fmt.Errorf("migration %d (%s): %w", m.version, m.desc, err)
			}
		} else {
...
func applyMigration109(tx *sql.Tx) error {
