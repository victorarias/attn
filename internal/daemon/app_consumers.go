...
func (d *Daemon) attributeWedgedDispatch(ctx context.Context, runtime *appRuntimeConnection, name string) error {
	...
	if err == nil {
	...
	} else if culprit, ok := d.wedgedAppCulprit(); ok && culprit == name {
		outcome = context.DeadlineExceeded
	} else if ok {
		...
		// The generation may already be exiting under another timed-out dispatch.
		// With no entered handler left to name, this waiter cannot honestly charge
		// an app for the shared runtime's failure.
		outcome = runtimeFailure("the app runtime stopped answering and generation %d had no entered handler left to attribute", runtime.generation)
	}
...
func (d *Daemon) noteAppFailure(name string, ev bus.Event, message string) {
	stalled, attempts, disable := d.noteAppStall(name, "subscription", ev.Seq, ev.Name, 0, message)
	if !disable {
		return
	}
...
func (d *Daemon) noteAppReconcileFailure(name string, claim store.AppReconcileClaim, message string) {
	stalled, attempts, disable := d.noteAppStall(
		name, "reconcile", claim.ThroughRequestID, "", claim.ThroughRequestID, message)
	if !disable {
		return
	}
