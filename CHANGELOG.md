...
### Fixed
- **Clicking a Notebook note opens it instantly.** Switching notes no longer briefly leaves the previous note's text on screen under the newly-selected one. The note you click now renders right away; its "linked from" backlinks (which scan the whole notebook) fill in a moment later instead of holding the content back.
