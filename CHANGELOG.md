...
- **A note's frontmatter shows as a card, not raw YAML.** When a note opens with a `---` properties block, it now renders as a tidy card at the top — title, type, summary, tags, sources, and dates — instead of a wall of `key: value` lines. Click the card to drop into the raw YAML and edit it; click back into the note and the card returns. The file on disk stays exactly the same plain markdown.
