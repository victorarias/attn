...
### Added
- **See what attn's background work is doing, from the app or `attn notebook tasks`.** A new Tasks section in the Notebook browser — and a matching command — lists attn's durable background tasks — the context-compaction, session-digest, journal-narration, and nightly memory-harvest jobs — showing each one's state, what it's working on, how many attempts it has taken, when it next runs, and the last error if it failed. The panel refreshes live as tasks change, and a stuck (failed or dead) task gets a Retry button to run it again now. It's a window into work that used to be invisible, so you can tell at a glance whether something is queued, running, retrying, or stuck.
...
### Fixed
- **A workspace or session can no longer corrupt your journal or other files.** Every internal narration path — the context snapshot taken at removal, each per-session digest, and the journal-narration pass itself — now refuses any workspace or session identifier that is not a plain name, so a crafted identifier can no longer steer a write out of attn's internal holding area and overwrite your curated journal or another file, nor point the narrator at a file outside its sandbox. Normal workspaces and sessions are unaffected.
