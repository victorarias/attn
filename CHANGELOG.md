...
- **Reconciliation verdicts now get a ground-truth check against tracked PRs.** When an orphaned-ticket reconciliation verdict mentions a pull request attn already knows is merged or closed, the posted comment gets an extra "Ground-truth check" line flagging that the verdict text may be stale on that point — so a verdict claiming "merge pending" hours after the PR actually merged doesn't stand unchallenged.
