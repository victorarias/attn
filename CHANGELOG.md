...
### Changed
- **Ticket nudges now wait, show themselves, and never land mid-sentence.** When an idle agent has unread ticket activity, attn no longer types the nudge into it instantly. Instead a short countdown (default 30s) runs first, shown as a bar on that session's sidebar row and a strip across its tile, so you can see a nudge coming. The session you're currently looking at never auto-fires — its countdown pauses and offers a "deliver now" button, and switching away resumes it — and a nudge is never typed into a session you've touched in the last few seconds, so it can no longer splice onto a prompt you're still typing and submit the garbled line. Sessions that are working (not idle) just show a quiet "unread activity" marker, with no countdown.
