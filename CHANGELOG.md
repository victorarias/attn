...
- **Agents can open interactive code tours inside attn.** Tours use a fullscreen reading workspace with a first-open briefing, semantic chapters, coverage and progress, searchable large-change navigation, authored review hotspots, resume and next-unreviewed controls, polished Markdown and Mermaid explanations, inline file annotations, Jaunt-style keyboard navigation, and a Tour-wide live conversation drawer that stays hidden until needed. Changed Markdown files open as rendered documents with a one-click switch to a line-numbered, commentable source diff. Escape closes the topmost Tour layer and then dismisses the Tour without ending it, and returning to a workspace does not reopen a Tour you already dismissed. Guides live in the active attn profile rather than the repository, include committed and uncommitted branch changes, persist across restarts, follow the app's custom font size, and keep the agent listening for questions and incremental feedback until the user explicitly ends the tour. Agents can use either a streaming listener or a one-event, sequence-cursor listener for runtimes that resume only when a command exits. Tour feedback remains local and is never submitted to GitHub.
...
### Fixed
- **Tour briefings and reviews are easier to use.** Briefing text now uses a larger reading size, Mermaid diagrams get a roomy scrollable canvas with zoom controls, the Tour uses one stable diff viewport so typing or receiving updates cannot knock the code back to another position, and the primary Tour header exposes review submission without requiring the conversation drawer to be open. Questions and review feedback now wake an idle listening agent with a durable event reference, so submissions are noticed without injecting their contents into the terminal.
