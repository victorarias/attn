...
- **Agents can open interactive code tours inside attn.** Tours present a curated summary, ordered files, skipped and other changes, polished Markdown and Mermaid explanations, inline annotations, local review notes, and an in-panel question thread in a wide reading layout that follows the app's custom font size. Guides live in the active attn profile rather than the repository, include committed and uncommitted branch changes, persist across restarts, and keep the agent listening for questions and incremental feedback until the user explicitly ends the tour. Tour feedback remains local and is never submitted to GitHub.
