...
- **Present manifests can now pin author annotations to specific lines.** A file entry can add `annotations`, each pinning a note or an ordered thread to a substring anchor, a single line, or a line range. attn resolves anchors against the pinned head commit when the presentation opens, so a broken anchor is caught immediately instead of silently failing later — `attn present validate` checks anchors locally too. An anchor that matches more than one line still opens, with a warning naming the ambiguity. Rendering these annotations in the reader is coming in a follow-up.
