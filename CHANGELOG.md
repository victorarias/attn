...
### Added
- **Archived GPUI Prototype**: Archived the GPUI native-client experiment after it established daemon-owned workspace layouts and the Ghostty external-I/O approach; the active Swift client now applies those lessons without inheriting the canvas/composition layer.
- **Native Workspace Launcher**: Added native `New Workspace` and directional `Add Pane` launchers with terminal or available coding-agent selection, saved pane/yolo preferences, daemon-backed directory and local repository/worktree destinations, atomic first-pane creation, and immediate `Cmd+D` / `Cmd+Shift+D` shell splits. Terminal-first workspaces are real root sessions, pane-created terminals can start in an edited directory, and background automation can inspect launcher state without taking foreground focus.
- **Background Native Automation**: Native-client automation now parks its test window at the screen edge with only a narrow strip visible while exercising Ghostty terminal rendering and readback. Assertions that require real AppKit keyboard events are guarded as an explicit interactive-only run because they necessarily take focus briefly, then return focus to the application that was in use.
- **Stable Local macOS Signing**: Source and native development builds now use an installed Apple Development signing identity when available, `make dev-native` packages the Swift client as a stable `com.attn.native.dev` app bundle for native-owned screenshots, and physical-input automation uses a signed `com.attn.harness.input-driver` app bundle so Screen Recording and Accessibility grants attach to stable identities; environments without a local identity continue to fall back to ad-hoc signing.
...
- **Native Terminal Mouse Input**: Drag-to-copy selection now starts where the mouse is pressed instead of a previously hovered cell, and mouse-aware terminal UIs receive correctly positioned click and drag events through the embedded Ghostty surface.
- **Native Split Cursor Focus**: Inactive native terminal panes no longer retain Ghostty's default focused cursor state and now mirror Ghostty.app's inactive-split dimming. Directly launched Claude Code panes also replay their startup terminal modes into Ghostty with historical writes suppressed, so they receive focus-loss events and hide the prompt cursor like Claude launched from a shell pane without replaying old terminal queries into the live PTY.
- **Native Quick Split Focus**: A shell pane created with `Cmd+D` or `Cmd+Shift+D` now becomes the input target after its native Ghostty view attaches, so typing continues in the new split without an extra click.
- **Native Pane Close Rendering**: Closing a split pane no longer briefly blanks remaining terminals while the layout collapses; mounted Ghostty surfaces remain alive and render continuously in their new positions.
- **Native Pane Navigation**: `Cmd+Option+Arrow` now moves focus between native terminal panes by their split layout position and switches to the previous or next workspace when movement reaches a pane edge.
- **Native Empty Workspace Recovery**: A workspace whose last session has ended now remains usable in the native client: it can be closed, escaped with pane-navigation shortcuts, or given a new first terminal or agent pane instead of becoming a stuck sidebar entry.
- **Native Dev Window Placement**: Opening the normal Swift development app now recenters a window left mostly off-screen by a prior parked automation run, without resetting an ordinarily visible user-positioned window.
- **Native Dev Client Lifecycle**: `make dev-native` now stops the dev-profile Swift client it launched when interrupted with `Ctrl-C`, and native clients enforce one running window owner per profile while still allowing isolated automation profiles to execute independently.
- **Native Split Sizing And Close Flow**: Nested quick shell splits now preserve the selected pane's half-sized branch instead of collapsing its outer column, `Cmd+W` closes panes and then workspaces before closing an empty window, closing a pane returns focus to the previously selected pane in that workspace, and closing the final window exits the native process.
- **Native Launcher Shortcuts And Scale**: `Cmd+Shift+N` now opens New Workspace, while horizontal Add Pane moves to `Cmd+Option+N`; the launcher modal and controls are scaled down for normal desktop use.
- **Native Launcher Parity**: New Workspace and Add Pane now remember Terminal or the last selected coding agent, expose visible `Option+1` through `Option+5` type shortcuts beginning with Terminal, put YOLO selection on the Local target card with capability checks, show ghost-text path completion accepted with Tab, support keyboard navigation through locations and worktrees including opening the highlighted folder with Return, default a new pane to the focused Ghostty terminal's reported working directory when available, present a compact command-palette-style picker without duplicating the selected path in Recent and Directories, and create worktrees inline from the selected checkout branch or the repository default branch with automatic branch-input focus, scoped Escape cancellation, and a single local progress indicator.
- **Native Terminal Clipboard**: Embedded Ghostty panes now support normal macOS `Cmd+C` and `Cmd+V` behavior through Ghostty's selection and paste pipeline. Clipboard behavior is covered by off-screen daemon-backed terminal automation without stealing desktop focus; protected paste remains blocked until its native confirmation UI exists.
- **Native Terminal Scrolling**: Trackpad and Magic Mouse scrolling in native Ghostty panes now carries Ghostty's precise-scroll and momentum metadata, so smooth scrolling is normalized correctly instead of racing through terminal history as discrete wheel ticks.
- **Native Agent Pane Startup**: Coding-agent panes launched inside a dev-profile workspace no longer print attn's profile-routing diagnostic into terminal content; explicit dev-profile CLI commands still show their routing banner.
- **Native Repository Destination Navigation**: Selecting a Git repository now immediately enables arrow-key navigation through its checkout/worktree destinations without tabbing into the list; the compact launcher keeps the active row scrolled into view while `Create worktree...` remains pinned and a bottom fade signals additional destinations below.
- **Native Worktree Selector Highlight**: Highlighting `Create worktree...` no longer leaves the previously navigated branch visually active; it remains available only as the source branch for creating the worktree.
- **Native Launcher Back Focus**: Returning from repository destination selection with Escape now restores focus to the location input, so a second Escape closes the launcher normally.
- **Native Workspace Close Latency**: Closing worker-backed native workspaces now removes daemon state immediately while the worker terminates an unresponsive TUI, rather than blocking each `Cmd+W` close behind a synchronous signal timeout.
- **Native Launcher Home Paths**: The native location picker now keeps paths inside the local home directory abbreviated with `~` when selected or completed, while still resolving full paths when launching panes and preserving explicitly typed absolute paths.
- **Native Location Browse Errors**: Typing or inspecting a path whose parent cannot be browsed no longer disconnects the native client with a missing-data error; the daemon now returns a valid empty suggestion result alongside the browse failure.
- **Native Launcher Text Focus**: Opening the workspace or pane launcher now leaves the location field focused with its caret at the end, instead of selecting the existing path and replacing it on the first keystroke.
- **Native Launcher Live Filtering**: Typing a location in the native launcher now shows the directory currently being browsed and refreshes its matching child-directory rows as the path moves deeper.
- **Native Launcher Shutdown**: Closing or interrupting the Swift native app while a workspace/pane launcher is open now dismisses the launcher and exits cleanly instead of leaving a modal sheet blocking shutdown with repeated system beeps.
- **Native Pane Close Recovery**: Closing panes after the dev daemon has recovered existing workspaces no longer crashes the daemon when the in-memory previously focused pane history has not yet been rebuilt.
- **Daemon Failure Diagnostics**: Detached daemons now retain process-level output and panic traces in a profile-specific `daemon.stderr.log`, so an unexpected native-client daemon loss can be diagnosed after the UI disconnects.

### Changed
- **Native Swift Terminal Foundation**: The replacement native client now renders daemon-owned workspace terminals through Ghostty's native external-I/O Metal surface, routes terminal input and resize callbacks back to daemon PTYs, and exposes background automation that verifies shell plus Claude Code and Codex terminal startup without taking over the active desktop.
