...
- **Agents can open interactive code tours inside attn.** Tours use a fullscreen reading workspace with a first-open briefing, semantic chapters, coverage and progress, searchable large-change navigation, authored review hotspots, resume and next-unreviewed controls, polished Markdown and Mermaid explanations, inline annotations, local review notes, and a conversation drawer that stays hidden until needed. Escape closes the topmost Tour layer and then dismisses the Tour without ending it, and returning to a workspace does not reopen a Tour you already dismissed. Guides live in the active attn profile rather than the repository, include committed and uncommitted branch changes, persist across restarts, follow the app's custom font size, and keep the agent listening for questions and incremental feedback until the user explicitly ends the tour. Tour feedback remains local and is never submitted to GitHub.
