...
- **`attn dispatch watch <id>` streams a dispatch's meaningful events and exits when it's done.** A new blocking command that prints one line per event worth acting on — a completion (with the agent's summary and next action), a genuine blocker or decision-request, or a failure/crash — and nothing for routine mid-work tool-permission prompts. It exits when the dispatch reaches a terminal state, covering every way one can end (completed, failed, or a session that died without reporting), so it never hangs silently. This is the event primitive a chief-of-staff agent watches per delegation instead of polling.
