...
### Changed
- **The chief of staff hands work off and stops, instead of hovering.** The chief's standing instructions now make its loop explicit: once it has delegated, recorded the work, and reported back, its turn is done — it follows progress on the ticket board rather than polling a working agent, and re-engages when you ask or a ticket needs input. It acts on reversible coordination on its own but checks with you before higher-impact moves like fanning out several agents or creating workspaces, and it verifies an important result against the actual deliverable a brief named rather than trusting a status comment.
