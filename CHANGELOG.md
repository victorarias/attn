...
### Fixed
- **Same-Size Redraw Cleanup**: Stop treating same-size terminal `fit()` and visibility-return events as reasons to request PTY redraw work, so ordinary no-geometry-change pane churn no longer carries a dedicated redraw path on top of the settled resize flow.
- **Same-App Resize Cleanup**: Remove the deferred non-shell redraw bounce from same-app pane resize handling, so split/open/close recovery relies on the settled PTY resize instead of a follow-up bounce/restore heuristic. Local split-close recovery canaries and the full packaged-app serial matrix still pass after the simplification.
...
- **Claude Relaunch Replay Policy**: Keep bounded raw relaunch replay targeted to Codex and restore fresh screen snapshots for Claude sessions, so remote Claude relaunch-close recovery does not get stuck on stale wrapped scrollback after the app reopens.
- **Harness Version-Mismatch Detection**: Packaged-app harness startup now checks `get_state` before treating the UI as responsive, so protocol mismatch banners like `daemon v50 / app v49` fail fast instead of masquerading as scenario-specific terminal regressions.
- **Same-App Remount Arming**: Pane runtime hydration now requires an actual pane detach before it will use the `same_app_remount` attach path, so ordinary binder rewires do not carry remount-restore machinery into stable same-app split churn.
- **Remote Claude Relaunch Canary Stability**: `TR-205` now treats Claude's final post-relaunch welcome/header recovery as a Claude-specific visibility check instead of exact line-anchor preservation, so the canary still catches width/paint/token regressions without flaking on benign welcome-screen reflow.
...
- **Agent Relaunch Replay Preference**: Frontend attach planning now carries agent-aware replay preference through relaunch attach classification, so Codex relaunches prefer bounded raw scrollback over fresh `screen_snapshot` replay when both are present, while Claude and shell relaunches keep the snapshot restore path.
- **Frontend Replay Gating Cleanup**: Derive attach replay handling directly from attach policy plus reported geometry instead of carrying separate client-side skip flags, so same-app remounts and fresh spawns cannot accidentally inherit relaunch reset/replay behavior from stale local state.
- **Existing Runtime Attach Path Cleanup**: Frontend PTY attach now routes relaunch restore and same-app remount through one policy-driven helper instead of maintaining separate near-identical attach flows, reducing duplication in the terminal rendering path without changing replay or geometry ownership rules.
- **Shared PTY Attach Planning**: Replay classification and geometry-reconcile decisions now live in a dedicated PTY planning utility instead of inside the daemon socket hook, so terminal rendering policy is easier to reason about and test in isolation.
- **Shared PTY Attach Result Semantics**: Reset decisions, replay application, queued attach-output draining, and truncated-restore warnings now run through shared PTY attach-result planning instead of being embedded inside websocket event handling, further shrinking rendering logic inside the socket hook.
- **Shared PTY Transport State**: Frontend PTY attachment ownership, attach-time queued output, attach context, and live sequence tracking now live behind one transport-state helper instead of four loosely-related refs in the websocket hook, while keeping desync recovery limited to stream state rather than silently detaching runtimes.
- **Shared PTY Runtime Lifecycle**: Frontend PTY spawn, daemon-known reattach, Claude resume recovery, remote utility-pane respawn, and fresh-shell bootstrap decisions now run through a dedicated runtime lifecycle helper instead of living inline inside the websocket hook, with focused tests covering which fallback branches remain necessary.
- **Shared PTY Geometry Lifecycle**: Frontend terminal settle-delay merging, remount hydration attach decisions, committed-geometry change detection, deferred redraw-after-resize checks, and redraw-skip-on-fresh-output logic now run through a dedicated geometry lifecycle helper instead of being embedded inside the pane runtime binder.
- **Pane Runtime Lifecycle Consolidation**: The pane runtime binder now keeps same-app remount state, cached spawn args, committed geometry, pending geometry sync, and geometry timer ownership inside one per-pane lifecycle record instead of parallel maps, reducing split/remount bookkeeping without changing terminal restore policy.
- **Pane Runtime Local State Consolidation**: Deferred unmount cleanup and terminal write-chain bookkeeping now also live inside the pane runtime lifecycle record instead of separate per-pane maps, so the binder's remaining standalone refs are mostly the live xterm/handle/subscription bindings rather than duplicated lifecycle state.
- **Pane Runtime Queue/Subscription Consolidation**: Queued PTY event backlog and pane input-subscription disposal now also live inside the pane runtime lifecycle record instead of separate pane maps, further shrinking binder-local bookkeeping without changing PTY replay or input behavior.
- **Pane Runtime Lifecycle Registry**: Pane-local runtime bookkeeping now lives in a dedicated `paneRuntimeLifecycleState.ts` helper with focused unit tests, so `usePaneRuntimeBinder.ts` keeps the orchestration path while lifecycle map/timer/subscription mechanics move out of the main binder file.
- **Pane Runtime Module Split**: Terminal key/input utilities, runtime ensure retry flow, runtime binding synchronization, pane controls, and inactive-pane pruning now live in dedicated helpers alongside the lifecycle registry, so `usePaneRuntimeBinder.ts` is smaller and closer to a pure orchestration layer instead of a catch-all runtime file.
- **Shared Terminal Viewport Actions**: Pane focus acquisition, programmatic scroll-to-top, and terminal reset now run through small viewport helpers instead of being open-coded inside the pane runtime binder, with focused tests proving scroll-pin reset is still required to flush buffered writes after the user pins scrollback.
- **Terminal Ready/Resize De-duplication**: `Terminal.tsx` now applies fit fallback, ResizeObserver readiness, and pre-ready font-size changes through one measured-geometry path, eliminating the old pre-ready font-change double-resize while preserving same-size fit redraw behavior.
- **Serial Matrix Cleanup Discipline**: The packaged-app relaunch and local close-redraw scenarios now tear down their sessions, endpoints, and app process in `finally`, and the automation client now tolerates transient missing manifests during app handoff, reducing cross-scenario matrix contamination.
- **Harness Close Fidelity**: Matrix scenarios now attempt cleanup through the app's own `close_session` path before falling back to direct daemon kills, so the packaged-app runner exercises the same close semantics users hit while still keeping best-effort cleanup for harness determinism.
...
- **Codex Relaunch Restore Divergence**: Codex relaunch attaches now keep the fresh screen snapshot whenever the bounded raw replay tail does not reconstruct the same visible frame, and the frontend no longer suppresses a snapshot-only Codex restore just because raw replay would have been preferred if it existed. This fixes the proved relaunch bug where the boxed Codex header could disappear or the pane could restore blank because raw and snapshot paths disagreed.
- **Geometry-Aware Codex Relaunch Replay**: Codex relaunch restore now prefers full geometry-segmented raw replay only when it matches the live screen model, rejects transport-clipped raw tails as state-unsafe, and falls back to a geometry-aware derived snapshot when no fresh live snapshot exists. The packaged-app `TR-205` and `TR-502` relaunch scenarios both restore through segmented replay and keep the Codex header intact.
- **Isolated Remote Harness Teardown**: Removing an isolated remote endpoint now best-effort stops the remote harness daemon, packaged-app remote scenarios sweep stale harness-root process trees before starting, and `TR-504` now proves both `close_session` worker cleanup and `remove_endpoint` daemon/relay teardown so stale remote Codex processes stop poisoning later matrix runs.
- **Real-App Paste Driver Stability**: The macOS harness input driver now keeps staged clipboard contents alive until the system paste completes, and `TR-502` warms the driver before timing typed echo, so bracketed-paste injections no longer intermittently arrive empty or fail the echo budget because the first Swift build happened inside the measurement window.
- **Relaunch Polling Tolerance**: Packaged-app pane/workspace polling now retries transient `Session not found` errors during app relaunch instead of treating the first missing-session probe as fatal, so remote relaunch scenarios wait through endpoint/session rehydration before declaring failure.
