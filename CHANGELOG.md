...
### Fixed
- **Zooming the active pane (⇧⌘Z) now works again in the packaged app.** The macOS Redo menu item was claiming the key equivalent before it could reach the app; attn now replaces the native Redo entry with its own "Zoom Pane" menu item so the shortcut is delivered correctly.
