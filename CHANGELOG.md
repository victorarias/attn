...
- **Selecting a command block taller than the screen no longer draws a box around the whole terminal.** A long command's block (e.g. `make`) used to clamp its selection border to the viewport edges, making it look like one giant box had swallowed the terminal. Off-screen block edges are no longer drawn: the highlight now shows side rails that read as "this block continues above/below."
